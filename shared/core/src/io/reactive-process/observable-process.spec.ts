...
describe('.toObservableProcess()', () => {
  it('should get one message and then done', done => {
    const ps = spawn(`echo`, ['foo']);

    const actualMessages: Array<any> = [];
    const actualErrors: Array<any> = [];
    toObservableProcess(ps).subscribe(
  ...
  it('should get four async messages in order, sterr, stdout, stderr and stdout again', done => {
    const ps = spawn(
      `node -e "setTimeout(() => console.error('error'), 50);
