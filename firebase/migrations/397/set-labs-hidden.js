...
module.exports = function (db, argv) {
  ...
  return db.ref('/labs')
    .orderByChild('common/hidden').equalTo(null)
    .once('value')
    .then(snapshot => snapshot.val())
    .then(labs => Object.entries(labs || {}))
    .then(entries => {
      if (entries.length === 0) {
        console.log(chalk.blue('No labs without `hidden` field found. Migration done.'));
      } else {
    ...
    .then(labs => {
      labs.forEach(lab => {
        console.log(chalk.blue(`Setting \`hidden\` field to \`false\` for lab: ${lab.common.id}`));
    ...
    .then(_ => {
      ...
      console.log(chalk.blue('Migration was done as dry-run. No database updates have been performed.'));
    });
}
