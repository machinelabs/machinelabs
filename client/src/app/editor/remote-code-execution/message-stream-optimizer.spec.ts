...
describe('MessageStreamOptimizer', () => {
  ...
  describe('.listenForMessages()', () => {
    it('should fetch entire messages if message count is below treshold', () => {
      ...
      let allMessages = Observable.from(availableMessages);

      spyOn(messageStreamOptimizer, 'getAllMessages').and.returnValue(allMessages);
      spyOn(messageStreamOptimizer, 'getTailMessage').and.returnValue(allMessages.last());
      ...
      let seenMessages = []
      messageStreamOptimizer.listenForMessages(1)
                .do(message => seenMessages.push(message))
                .finally(() => {
    ...
    it('should fetch head, tail and live messages if message count is below treshold', (done) => {
      ...
      let liveMessages = Observable.of(
      ...
      spyOn(messageStreamOptimizer, 'getAllMessages').and.returnValue(allMessages);
      spyOn(messageStreamOptimizer, 'getTailMessage').and.returnValue(allMessages.last());
      spyOn(messageStreamOptimizer, 'getHeadMessages').and.returnValue(allMessages.take(2));
      spyOn(messageStreamOptimizer, 'getTailMessages').and.returnValue(allMessages.skip(8));
