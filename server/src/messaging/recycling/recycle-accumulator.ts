              ...
              if (cmdInfo.patched === expectedPatchCount && cmdInfo.removed === this.config.deleteCount) {
                ...
                this.config.messageRepository
                    ...
                    .subscribe(() => {
                      ...
                      // Realtime API.
                      console.error(`RecycleId ${recycleId} at ${Date.now()}: Unexpected error during bulk update of message recycling for execution ${this.executionId}`);     // tslint:disable-line:max-line-length
                      console.error(err);
                    });
