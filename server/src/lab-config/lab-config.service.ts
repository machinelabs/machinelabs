...
export class LabConfigService {
  ...
  toInternalConfig(userId: string, publicConfig: PublicLabConfiguration) {
    ...
    .do(validatedMounts => config.mountPoints = this.mountService.extractValidMountPoints(validatedMounts))
    .do(validatedMounts => config.errors.push(...this.mountService.extractErrors(validatedMounts)))
    .let(mute)
    // We can't just map here because we may not have any mounts. Muting and concating works in that case, as well
    .concat(Observable.of(config))
  ...
  private hasValidInputs(config: InternalLabConfiguration) {
    return Array.isArray(config.inputs) && config.parameters.every(param => isObject(param) && !!param['name'] && !!param['url']);
  }
