...
.action(cmd => {
  fromPromise(firebase.auth().signInAnonymously())
    .pipe(
      switchMap(res => {
        return refBuilder.handshakeRequestRef(res.uid)
          .update({id: res.uid})
          .pipe(
            tap(_ => {
              console.log(chalk.default.yellow(`Opening default browser to login. Please accept request only if the ID shown in the browser matches: ${chalk.default.bold.green(res.uid)}`));
              open(`${environment.mlDomain}/handshake/${res.uid}`);
            }),
            switchMap(_ => refBuilder.handshakeRequestRef(res.uid).value()),
            map((snapshot: any) => snapshot.val()),
            filter((val: any) => val && !!val.token),
      ...
      switchMap((val: any) => loginAndCache(val.token))
    )
    .subscribe((val: any) => {
